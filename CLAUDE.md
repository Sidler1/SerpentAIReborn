...
It was brought from its mid-2020 state (Python 3.8, Poetry, torch 1.5+cu101, TensorFlow, Crossbar/WAMP, cefpython/kivy) up to **v0.1.0**: Python 3.13+, uv, PyTorch 2.x, pluggable transport, Wayland-aware. **`ROADMAP.md`** tracks what's planned next (toward 0.2.0 / 0.3.0) and what's intentionally deferred — read it before starting new work.
...
- **Transport** — the bus (frames, input, analytics) goes through `serpent/transport`. **Default is `in_process`** (no Redis): the grabber runs as a thread and input is driven directly — best for a single-machine agent run. Switch to `transport.backend: redis` (and run `redis-server`) for the multi-process model or to share the bus across processes (e.g. running the dashboard/visual-debugger alongside the agent, since in-process can't cross processes).
...
- **Tesseract** for OCR (`pytesseract`). Capture/input/window currently speak **X11**; on a **Wayland** session they work via **XWayland** (`$DISPLAY` must be set — true on KDE/GNOME by default), which covers Steam/Proton games. `WindowController` raises if Wayland is detected with no XWayland, and warns when falling back to XWayland. Native-Wayland backends (PipeWire capture, ydotool input, KWin/kdotool window control) are scaffolded but not implemented — see `serpent/window_controllers/wayland_window_controller.py` and ROADMAP.md.
- Plugins live in a top-level `plugins/` directory (offshoot discovers them there). The repo bundles three example game plugins: **Super Hexagon** is a full worked reference (modern `Game` + `GameAPI` + a real `PLAY` agent); **Binding of Isaac** and **You Must Build A Boat** are modern-API scaffolds (load/validate; their deep 2017 AI lives on `master` in git history and is a TODO port). Each game = a `Game` plugin (`screen_regions`, Steam launcher config, `api_class`) + a `GameAgent` plugin (frame handlers).
...
`serpent/machine_learning/reinforcement_learning/`: `agent.py` is the base `Agent` (handles the `game_inputs` → action-space mapping, the key abstraction — see `tests/unit/test_agent.py`). Concrete agents in `agents/`: `rainbow_dqn_agent`, `ppo_agent`, `random_agent`, `recorder_agent`. The Rainbow DQN and PPO implementations (`rainbow_dqn/`, `ppo/`) are **pure PyTorch (torch 2.x)** and **device-agnostic**: `serpent/machine_learning/device.py::get_device()` auto-selects CUDA/ROCm → MPS → CPU; agents call `.to(device)` (no hard-coded `.cuda()`/`set_default_tensor_type`). Full-model checkpoints load with `weights_only=False`. Context classification (`context_classification/`) is a **torchvision/PyTorch** `CNNContextClassifier` (ImageFolder-trained, device-agnostic) that replaced the dead TF/Keras Inception/Xception classifiers.
...
## Conventions & gotchas
- **Dependencies are modernized.** `pyproject.toml` carries a modern set incl. torch 2.x and fastapi/uvicorn; the dead 2020 stack — TensorFlow, crossbar/autobahn/twisted, cefpython3, kivy, pony, aioredis, comet_ml, lmdb, luminoth — is removed. Don't reintroduce them; check `ROADMAP.md` for what's intentionally absent.
...
- **Lint scope.** CI runs ruff over `tests/` (the modernized `serpent/` modules are written clean and verified ad-hoc, but `extend-exclude = ["serpent", "cli.py", "dashboard", "plugins"]` keeps legacy/example code from blocking CI). New code should be ruff-clean; verify a `serpent/` module explicitly with `uv run ruff check serpent/<path>`.
