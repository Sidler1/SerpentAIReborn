...
- **Input is a Redis bridge, not WAMP (as of Phase D).** The default `ClientInputController` pickles `(func_name, *args, kwargs)` onto the `SERPENT:INPUTS` list; `game.play()` spawns `serpent.input_controllers.redis_input_controller_worker`, which `brpop`s that list and dispatches to a real backend. WAMP/crossbar/autobahn and `serpent/wamp_components/` are **removed**; the analytics consumer + dashboard API are rebuilt on a modern transport in the dashboard phase (`AnalyticsClient` still produces events to Redis in the meantime).
</content>
